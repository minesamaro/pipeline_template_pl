...
from src.modules.model.multitasksembin.multitasksembin import MultiTaskSurvivalStageBinNet2D
from src.modules.loss_functions.multitask_loss_functions import MultitaskBinLossFunction
...
class PyTorchLightningMultitaskSEMBinModel(pytorch_lightning.LightningModule):
    ...
    def evaluate_on_test_set(self):
        ...
        with torch.no_grad():
            for batch in self.test_dataloader_ref:
                ...
                stage_probs = torch.sigmoid(stage_logits)
        ...
        test_labels = torch.cat(test_labels, dim=0).to(self.device).view(-1)
        ...
        test_stage_pred_class = (test_stage_preds > 0.5).int()
        ...
        metrics_for_logging = {
            ...
            'stage_test_balanced_accuracy0.5': balanced_accuracy_score(
                ...
                y_pred=test_stage_pred_class.cpu().numpy()
            ),
        }
        # Log to wandb
    ...
    def on_validation_epoch_end(self):
        ...
        val_stage_pred_classes = (predicted_stage_labels > 0.5).int()
